...
const Middle_Component = () => {
  ...
  useEffect(() => {
    ...
    fetch(
      "http://localhost:3001/api/v1/search?pageSize=100000&pageNumber=0",
      options
    )
      .then((response) => response.json())
      .then((response) => {
        console.log(response.items);
        setItemsSum(response.total);
  ...
  const handlePageClick = (event) => {
    ...
    console.log(
      `User requested page number ${event.selected}, which is offset ${newOffset}, currentItems:${currentItems.length}`
    );
